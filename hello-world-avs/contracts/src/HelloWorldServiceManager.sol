...
contract HelloWorldServiceManager is ECDSAServiceManagerBase, IHelloWorldServiceManager {
    ...
    function respondToTask(
        ...
        require(block.number <= task.deadline, "Task deadline has expired");

        // The message that was signed
        // bytes32 messageHash = keccak256(abi.encodePacked("Hello, ", task.name));
        bytes32 messageHash = keccak256(abi.encodePacked("Execute swap: ", task.tokenIn, task.tokenOut, task.amountIn, task.deadline));
